...
export function AuthModal({ onClose, onSuccess }: AuthModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Modal Container */}
      <div
        className={`relative w-full max-w-[420px] bg-[#000000] rounded-[24px] overflow-hidden flex flex-col transition-all duration-[400ms] ${
          mounted ? "opacity-100 translate-y-0 scale-100" : "opacity-0 translate-y-8 scale-[0.96]"
        }`}
        style={{
          boxShadow: "0 0 0 1px rgba(255,255,255,0.08), 0 30px 80px -20px rgba(0,0,0,1)",
        ...
        <div className="px-6 pb-8">
          ...
          <div className="mb-8 mt-2">
            <h2 className="text-2xl font-semibold text-white tracking-tight mb-2">
              {step === "identifier" ? "System Access" : "Verify Protocol"}
            </h2>
            <p className="text-[14px] text-[#a1a1aa] leading-relaxed">
              {step === "identifier" 
                ? "Enter your operator credentials or connect via secure ecosystem." 
                : <>Verification code dispatched to <span className="text-white font-medium">{identifier}</span></>}
            </p>
          </div>
          ...
          {step === "otp" && (
            <div className="animate-message-reveal flex flex-col items-center">
              <div className="flex justify-center gap-2 md:gap-3 mb-8 mt-4 w-full">
                {otp.map((digit, idx) => (
                  <input
                    ...
                    className="w-10 h-12 md:w-12 md:h-14 bg-transparent border-b-2 border-[#27272a] focus:border-[#00ff66] text-center text-2xl font-mono text-white outline-none transition-colors"
                  />
                ))}
              </div>
              ...
              <button 
                ...
                className="flex items-center justify-center gap-2 w-full bg-white text-black font-semibold py-3.5 rounded-xl hover:bg-gray-200 transition-all disabled:opacity-50 disabled:hover:bg-white cursor-pointer"
              >
                {isLoading ? <Loader2 className="size-5 text-black animate-spin" /> : <><KeyRound className="size-4" /> Verify Protocol</>}
              </button>
